...
# creating database hotel_db
login.execute("create database if not exists hotel_db")

# using database hotel_db
login.execute("use hotel_db")
...
def categorywise():
    try:
        ...
        if rooms:
            ...
            print(tabulate(rooms, headers=headers))
        else:
...
def roomsoccupied():
    ...
    try:
        ...
        if all_rooms:
            ...
            print(tabulate(all_rooms, headers=headers))
            ...
            if filter_option == 'yes':
                ...
                query_filtered = """
                        SELECT RoomID, CustomerName, OccupancyDate, VacancyDate 
                        FROM PreBooking 
                        WHERE DATE(OccupancyDate) <= DATE(NOW() + INTERVAL %s DAY) 
                ...
                if filtered_rooms:
                    ...
                    print(tabulate(filtered_rooms, headers=headers))
                else:
...
def newbooking():
    try:
        ...
        # Generate Booking ID
        booking_id = f"BK{str(room_id).zfill(3)}"
...
def bookingstatus():
    ...
    if bookings:
        ...
        if booking:
            ...
            print(tabulate([booking], headers=headers))
        else:
...
def viewbookings():
    ...
    if bookings:
        headers = ["Booking ID", "Customer Name", "Mobile Number", "Booking Date", "Occupancy Date", "Number of Days", "Vacancy Date", "Advance Payment", "Stay Cost", "Room ID"]
        print("All Bookings:")
        print(tabulate(bookings, headers=headers))
    else:
        print("No bookings available.")
